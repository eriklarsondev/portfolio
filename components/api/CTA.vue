<template>
  <div>
    <div
      class="relative lg:h-[45vw] lg:min-h-[600px] min-h-[unset] md:max-h-[800px] md:my-32 my-20 bg-zinc-900"
      v-if="data">
      <div
        class="absolute top-0 left-0 w-full h-full lg:bg-fixed bg-cover bg-left-top grayscale"
        :style="{ backgroundImage: `url(${data.image.url})` }"
        v-if="data.image"></div>

      <div class="absolute top-0 left-0 w-full h-full lg:bg-black/30 bg-black/50" v-if="data.image"></div>
      ...
      <div class="flex justify-center md:items-end items-center lg:absolute w-full h-full top-0 left-0">
        <Container class="relative py-20 sm:text-center">
          <Heading :cta="true" class="[&]:m-0 [&]:text-white/50" data-aos="fade-down" v-if="data.heading">
